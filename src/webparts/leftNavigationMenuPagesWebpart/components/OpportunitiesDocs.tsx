...
var currFavID = 0;
var thisDuplicating;
export default class OppoDocuments extends React.Component<IDisplayDocumentsProps, IDetailsListDocumentsExampleState> {
                    ...
                    <div className={styles.row}>
                        ...
                        <div className={'ms-Grid'} dir="ltr">
                            <div className="search ms-Grid-row">
                                <div className={'searchBocContains ' + styles.searchBocContains}>
                                    <div className="ms-SearchBoxExample">
                                        <div className={'ms-Grid-col ms-lg12 ' + styles.searchboxtitle}>Opportunities</div>
                                        <div className={'ms-Grid-col ms-lg12 ' + styles.margin}>
                                            <SearchBox
                                                className={'ms-Grid-col ms-sm8 ms-lg9 ' + styles.searchbox}
                                                placeholder="Search"
                                                onSearch={newvalue => this.opposearchresults(newvalue)}
                                                onFocus={() => console.log('onFocus called')}
                                                onBlur={onblurval => this.onBlurSearch(onblurval)}
                                                onChange={onchangeVal => this.onChangeSearch(onchangeVal)}
                                                spellCheck={false}
                                            />
                                            <PrimaryButton className={'ms-Grid-col ms-sm3 ms-lg2 ' + styles.button} text="Search" onClick={() => { this.opposearchresults(this.searchboxval); }} />
                            ...
                            <div className={'ms-Grid-row recentDocs ' + styles.recent}>
                                <div className={'ms-Grid-col ms-lg12 accordion1 ' + styles.accordion1}><p className={'ms-Grid-col ms-lg11 accordiontitle1 ' + styles.accordionTitle} >Recent Opportunities</p><div className={styles.accordionicon4}><i className={'ms-Grid-col ms-lg1 accordionicon1 ms-Icon ms-Icon--ChevronRight ' + styles.accordionicon1} aria-hidden="true"></i></div></div>
                            ...
                            <div className={'ms-Grid-row ClientFavourites ' + styles.ClientFavourites}>
                                <div className={'ms-Grid-col ms-lg12 accordion2 ' + styles.accordion2}><p className={'ms-Grid-col ms-lg11 accordiontitle2 ' + styles.accordionTitle} >Favourite Opportunities</p><div className={styles.accordionicon4}><i className={'ms-Grid-col ms-lg1 accordionicon2 ms-Icon ms-Icon--ChevronRight ' + styles.accordionicon2} aria-hidden="true"></i></div></div>
                                <div className={'containsoppoFav ' + styles.panel}>
                                    <MarqueeSelection selection={this._selection}>
                                        <DetailsList
                                            items={favorites}
                                            //compact={isCompactMode}
    ...
    public async opposearchresults(text: string) {
        if (text != undefined && text != "") {
            $('.opposearchResults').html('');
            let absUrl: string = this._context.pageContext.web.absoluteUrl + '/Opportunities';
            ...
            $.ajax({
                //url: absUrl + "/_api/search/query?querytext='(Title:"+text+"* OR PTLFriendlyName:" + text + "*) IsDocument:false + path:" + absUrl + "'&selectproperties='Title,Path,RefinableString07,PTLFriendlyName'&refiners=%27RefinableString07%27&rowlimit=500",
                url: absUrl + "/_api/search/query?querytext='(Title:"+text+"* OR PTLFriendlyName:" + text + "*)'&selectproperties='Title,Path,RefinableString07,PTLFriendlyName'&refiners=%27RefinableString07%27&rowlimit=500",
                method: "GET",
                headers: {
            ...
            success: function (recents) {
                ...
                if (recents.d.query.PrimaryQueryResult.RefinementResults != null) {
                    $.each(recents.d.query.PrimaryQueryResult.RefinementResults.Refiners.results[0].Entries.results, function (index, value) {
                        var hrefUrl = absurl + "/Opportunities/" + value.RefinementName +"?id="+ serverRelUrl +"/Opportunities/"+value.RefinementName;
    ...
    public async getOppoFavourites(para) {
        var htmlStr = "";
        _favorites = [];
        pnp.sp.web.currentUser.get().then(result => {
            let currUserId: number = result.Id;
            let queryStr: string = "Author eq " + currUserId + "and PTLCategory eq 'Opportunities'";
            pnp.sp.web.lists.getByTitle("Favourite")
                ...
                .then((data) => {
                    ...
                    $.each(data, function (index, value) {
                        // htmlStr += '<a href="'+value.PTLItemURL+'"><div id="favoritebox' + index + '" class="ms-Grid-col ms-lg5 favoritebox ' + styles.favoritebox + '"><div class="ms-Grid-col ms-lg2 partybox ' + styles.partybox + '"><i class="ms-Icon ms-Icon--PartyLeader" aria-hidden="true"></i></div><div class="ms-Grid-col ms-lg7 clientAnchorlib"><div class="clientAnchorlib ' + styles.clientAnchorlib + '">' + value.Title + '</div></div><div class="drillbox ms-Grid-col ms-lg3 drillbox ' + styles.drillbox + '"><i class="ms-Icon ms-Icon--DrillDown" aria-hidden="true"></i></div></div></a>';
                    ...
                    //$('.containsoppoFav').append(htmlStr);
